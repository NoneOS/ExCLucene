...
TextFragment** Highlighter::getBestTextFragments(
    ...
    try {
        ...
        //TCHAR substringBuffer[5 * LUCENE_MAX_WORD_LEN]; // FIX 2015-05-26
        // For GX150-84-11508862 in GOV2, we modified the stringbuffer length
        // Maybe we should filter such long terms during indexing
        //TCHAR substringBuffer[100 * LUCENE_MAX_WORD_LEN]; // FIX 2016-04-06, by Rui
        TCHAR *substringBuffer =new TCHAR[100 * LUCENE_MAX_WORD_LEN];
        // For ClueWeb Dataset and AOL query log, we modified the stringbuffer length
        ...
        while (tokenStream->next(&token)) {
            if ((tokenGroup->getNumTokens() > 0)&&(tokenGroup->isDistinct(&token))) {
                ...
                if (_textFragmenter->isNewFragment(&token)) {
                    currentFrag->setScore(_fragmentScorer->getFragmentScore()); // Bug Here
