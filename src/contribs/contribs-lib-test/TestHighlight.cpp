...
const TCHAR* hl_FIELD_NAME = _T("content");
...
void testSimpleHighlighter(CuTest *tc) {
    char* abuf = NULL;
    size_t r = 0, readlen, count = 0;
    TCHAR tbuf[1024];    

    FILE* f = fopen("/gov2.query.txt", "r");
    //while (count < 300 && (r = getline(&abuf, &readlen, f)) != -1) {
    if (count < 300 && (r = getline(&abuf, &readlen, f)) != -1) {

        abuf[r] = 0; STRCPY_AtoT(tbuf, abuf, r); tbuf[r] = 0;
        doSearching(tc, tbuf);
        uint64_t startTime = Misc::currentTimeMillis();

        QueryScorer scorer(hl_query);
        Highlighter highlighter(&scorer);
        SimpleFragmenter fragmenter(100);
        highlighter.setTextFragmenter(&fragmenter);
        int maxNumFragmentsRequired = 2;
        //    for (int i = 0; i < hl_hits->length(); i++) {
        size_t rsnum = hl_hits->length() > 500 ? 500 : hl_hits->length();
        for (int i = 0; i < rsnum; i++) {
            ...
            // CuMessage(tc, _T("%s\n"), result == NULL ? _T("") : result);
            std::wcout << result << std::endl << "-----------------------" << std::endl;
            _CLDELETE_CARRAY(result);
            _CLDELETE(tokenStream);
        }                
        totaltime += Misc::currentTimeMillis() - startTime;
        totalhits += hl_hits->length();
        abuf = NULL;
        
        count++;
    }
    fclose(f);
    std::cout<<"Q = "<<totalhits<<", T = "<<totaltime<<std::endl;
...
//    writer.optimize();
//    writer.close();
    hl_reader= IndexReader::open("/gov2-essentialDocs");
    hl_searcher = new IndexSearcher(hl_reader);
