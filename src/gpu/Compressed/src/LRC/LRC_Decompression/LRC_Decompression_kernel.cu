...
__global__ void LRC_Decompression(uint32_t *d_lists, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, regression_info_t *d_linearInfo)
{
	//shared
	...
	if (tid < activeThreadNum) {
		shortest_segment[tid] += int(eleIndex * fSlope) + iIntercept;
		shortest_list[eleIndex] = shortest_segment[tid];//write it in global memory
	}
	__syncthreads();
}
