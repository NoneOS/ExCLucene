...
#include "LRC_Intersection_kernel.cu"
...
string data_dir = "/media/indexDisk/naiyong/data/LRC/Compression/0/";
string regressioninfo_dir = "/media/indexDisk/naiyong/data/LRC/Generator/";
string result_dir = "/media/indexDisk/naiyong/result/LRC/Intersection/";
...
void allocateResource() {
	...
	CUDA_SAFE_CALL(cudaMallocHost((void**)&h_linearInfo, 16380 * sizeof(regression_info_t)));	
	...
	CUDA_SAFE_CALL(cudaMalloc((void**)&d_linearInfo, 16380 * sizeof(regression_info_t)));		
...
batchInfo CPUPreprocess(unsigned char *&ptr_in, const unsigned char *ptr_end) {
	...
	uint32_t LinearInfo_offset = 0;	// offset for h_linearInfo
	...
	while (ucntShortest_sum <= Threshold && ptr_in < ptr_end) {
		...
		// fill the LinearInfo
		for (uint32_t i = 0; i < tcount; ++i) {
			h_linearInfo[LinearInfo_offset + i] = h_regression_info[tc.tno[i]];
		}
	...
	bi.nTotalQueryNum = nTotalQueryNum;
	bi.nLinearInfoInByte = LinearInfo_offset * sizeof(regression_info_t);
...
void htodTransfer(const batchInfo &bi) {
	...
	CUDA_SAFE_CALL(cudaMemcpy(d_linearInfo, h_linearInfo, bi.nLinearInfoInByte, cudaMemcpyHostToDevice));
}
...
#endif
	LRC_Intersection<<<bi.blockNum, THREAD_NUM>>>(d_lists, d_isCommon, d_queryID_perBlock, d_shortest_lists, d_segOffset, d_segHead, d_linearInfo);
...
void releaseResource() {
	...
	CUDA_SAFE_CALL(cudaFreeHost(h_linearInfo));
	h_linearInfo = NULL;
	...
	CUDA_SAFE_CALL(cudaFree(d_linearInfo));
	d_linearInfo = NULL;
...
void Run(const string &dataset, unsigned char *ptr_in, unsigned char *ptr_end) {
	string result = result_dir + dataset + "_LRC_Intersection.txt";
