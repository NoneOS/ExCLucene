...
struct batchInfo {
	...
	uint32_t nLinearInfoInByte;	 // bytes d_linearInfo needs to transfer
};
...
int *h_segmin;
int *d_segmin;		
uint32_t segmin_size;

// regression info
regression_info_t *h_linearInfo;						
regression_info_t *d_linearInfo;			
...
__global__ void LRC_Intersection(uint32_t *d_lists, uint32_t *d_isCommon, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, uint32_t *d_segHead, regression_info_t *d_linearInfo) {
	//shared
	__shared__ uint32_t s_array[256];
	__shared__ regression_info_t s_linearInfo[256];
	...
	// get linearInfo
	if (tid < tcount) {
		s_linearInfo[tid] = d_linearInfo[s_array[1] + tid];
	}
	__syncthreads();
	...
	float fSlope = s_linearInfo[0].fSlope;		
	int iIntercept = s_linearInfo[0].iIntercept;
	float fSearchRange = 0;
	uint32_t DocID = 0;

	if (tid < activeThreadNum) {
		uint64_t codeword = d_lists[idx] | (static_cast<uint64_t>(d_lists[idx + 1]) << 32);
		DocID = (codeword >> shift) & mask;
		DocID += int(eleIndex * fSlope) + iIntercept;
	...
	if (tid < activeThreadNum) {
		...
		for (uint32_t listIdx = 1; listIdx < tcount; ++listIdx) {
			...
			fSlope = s_linearInfo[listIdx].fSlope;
			iIntercept = s_linearInfo[listIdx].iIntercept;
			fSearchRange = s_linearInfo[listIdx].fSearchRange;
			float pivot = (DocID - iIntercept) / fSlope;   // slow
//			float pivot = __fdividef(DocID - iIntercept, fSlope);
			int leftBorder = int(pivot - fSearchRange);
			left = leftBorder < 0 ? 0 : leftBorder >> THREAD_NUM_BIT;

			right = (ucnts[listIdx] - 1) >> THREAD_NUM_BIT;
			...
			*/
			...
			while (left <= right) {
				...
				q += int((start_eleindex + middle) * fSlope) + iIntercept;
			...
			if (!found)				//not exist, set isCommon to 0 and break
			{
				isCommon[eleIndex] = 0;
				break;
			}
...
__global__ void LRC_Intersection2(uint32_t *d_lists, uint32_t *d_isCommon, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, uint32_t *d_segHead, float *d_linearInfo)
{
	//shared
	__shared__ uint32_t s_array[256];
	__shared__ float s_linearInfo[256];
	__shared__ uint32_t activeThreadNum;//thread number active in the block
	__shared__ uint32_t startIdx;		//start index of the shortest list in the query for this block
	__shared__ uint32_t s_tcount;		//tcount in shared
	__shared__ uint32_t constantOffset;	//segment offset from the head of constant memory
	...
	int nBase = s_linearInfo[4 * tcount];	//base of shortest list
	uint32_t DocID = 0;

	if (tid < activeThreadNum) {
		uint64_t codeword = d_lists[idx] | (static_cast<uint64_t>(d_lists[idx + 1]) << 32);
		DocID = (codeword >> shift) & mask;
		DocID += (int)(eleIndex * fSlope + fIntercept) + nBase;
...
//			uint32_t pivot = (s_head - fIntercept) / fSlope;   // slow
	...
	int middle, left, right;	// for binary search
	uint32_t q;			  	    // DocID in longer lists
	uint32_t segment_start;		// start of corresponding segment in current longer list
	uint32_t segment_id;
	uint32_t segment_eleid;
	...
	if (tid < activeThreadNum) {
		...
		for (uint32_t listIdx = 1; listIdx < tcount; ++listIdx) {
			...
			nBase = s_linearInfo[(tcount << 2) + listIdx];

			left = s_Border[listIdx << 1];
			right = s_Border[(listIdx << 1) + 1];
			while ((left >> THREAD_NUM_BIT) < (right >> THREAD_NUM_BIT)) {
				middle = (left + right) >> 1;
				segment_id = middle >> THREAD_NUM_BIT;
				segment_eleid = middle & (THREAD_NUM - 1);
				
				// calculate the lb,eb,hb and en descriptors of the segment just found
				segment_start = uwlists[listIdx] + (d_segOffset + segNum[listIdx])[segment_id];
				descriptor = *(d_lists + segment_start);
				++segment_start;		
				lb = (descriptor >> 26) % 32;
				mask = (1U << lb) - 1;

				// decompress horizontal distance
				idx = segment_start + ((segment_eleid * lb) >> 5);
				shift = (segment_eleid * lb) & 0x1f;
			...
			// calculate the lb,eb,hb and en descriptors of the segment just found
			segment_start = uwlists[listIdx] + (d_segOffset + segNum[listIdx])[left >> THREAD_NUM_BIT];
			descriptor = *(d_lists + segment_start);
			++segment_start;			
			lb = (descriptor >> 26) % 32;
			mask = (1U << lb) - 1;
