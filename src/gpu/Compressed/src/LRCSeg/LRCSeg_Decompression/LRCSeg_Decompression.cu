...
#include "LRCSeg_Decompression_kernel.cu"

using namespace std;


string dataset_dir = "/media/indexDisk/naiyong/dataset/";
string data_dir = "/media/indexDisk/naiyong/data/LRCSeg/Compression/";
string regressioninfo_dir = "/media/indexDisk/naiyong/data/LRCSeg/Generator/";
string result_dir = "/media/indexDisk/naiyong/result/LRCSeg/Decompression/";
...
void kernelInvoke(const batchInfo &bi) {
	...
	LRCSeg_Decompression<<<bi.blockNum, THREAD_NUM>>>(d_lists, d_queryID_perBlock, d_shortest_lists, d_segOffset, d_base, d_fLinearInfo);
...
void runTest(int argc, char** argv) {
	...
	FILE *foffset = fopen(segNum_file, "rb");
	if (!foffset) {
	...
	readFile((unsigned char *)h_segNum, foffset);
	fclose(foffset);
...
int main(int argc, char** argv)  {
	...
	string dataset = argv[1];
	cout << "LRCSeg_Decompression" << endl;
	cout << "dataset = " << dataset << endl;

	string result = result_dir + dataset + "_LRCSeg_Decompression.txt";
