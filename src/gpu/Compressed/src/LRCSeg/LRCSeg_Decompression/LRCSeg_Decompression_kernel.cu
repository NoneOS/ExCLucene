...
__global__ void LRCSeg_Decompression(uint32_t *d_lists, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, int *d_base, float *d_fLinearInfo)
{
	//shared
	...
	if (tid < activeThreadNum) {
		shortest_segment[tid] += (int)(eleIndex * fSlope + fIntercept) + nBase;
