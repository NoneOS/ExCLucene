...
#include "LRCSeg_Intersection_kernel.cu"

using namespace std;


string dataset_dir = "/media/indexDisk/naiyong/dataset/";
string data_dir = "/media/indexDisk/naiyong/data/LRCSeg/Compression/0/";
string regressioninfo_dir = "/media/indexDisk/naiyong/data/LRCSeg/Generator/";
string result_dir = "/media/indexDisk/naiyong/result/LRCSeg/Intersection/";
...
batchInfo CPUPreprocess(unsigned char *&ptr_in, const unsigned char *ptr_end) {
	...
	while (ucntShortest_sum <= Threshold && ptr_in < ptr_end) {
		...
		for (uint32_t i = 0; i < tcount; ++i) {
			...
			tc.segNum[i] = *(h_offset + input.tno[i]);
		}
		...
		// fill the LinearInfo
		for (uint32_t i = 0; i < tcount; ++i) {
			h_fLinearInfo[i + LinearInfo_offset] = (h_regression_info + tc.tno[i])->fSlope;
			h_fLinearInfo[tcount + i + LinearInfo_offset] = (h_regression_info + tc.tno[i])->fIntercept;
			h_fLinearInfo[tcount * 2 + i + LinearInfo_offset] = (h_regression_info + tc.tno[i])->nRangeLeft;
			h_fLinearInfo[tcount * 3 + i + LinearInfo_offset] = (h_regression_info + tc.tno[i])->nRangeRight;
		}
...
void kernelInvoke(const batchInfo &bi) {
#ifdef debug	
	cout << "blockNum" << bi.blockNum << endl;
#endif
	
	LRCSeg_Intersection<<<bi.blockNum, THREAD_NUM>>>(d_lists, d_isCommon, d_queryID_perBlock, d_shortest_lists, d_segOffset, d_segHead, d_base, d_fLinearInfo);
	...
	FILE *fResultSum;
	char ResultSum[512];
	sprintf(ResultSum, "ResultSum_a_%u.txt", Threshold / (SM_NUM * THREAD_NUM));
	...
	FILE *fResultNum;
	char ResultNum[512];
	const uint32_t SM_NUM = 15;
	sprintf(ResultNum, "ResultNum_a_%u.txt", Threshold / (SM_NUM * THREAD_NUM));
...
void Run(const string &dataset, unsigned char *ptr_in, unsigned char *ptr_end) {
	string result = result_dir + dataset + "_LRCSeg_Intersection.txt";
...
void runTest(int argc, char **argv) {
	...
	FILE *foffset = fopen(segNum_file.c_str(), "rb");
	if (!foffset) {
