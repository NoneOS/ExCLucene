...
__global__ void ParaPFD_Decompression(uint32_t *d_lists, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, uint32_t *d_segHead) {
	// shared
	...
	if (tid < activeThreadNum) {
		shortest_list[eleIndex] = shortest_segment[pout * THREAD_NUM + tid];
	}
	__syncthreads();
}
