...
__global__ void ParaPFD_Intersection(uint32_t *d_lists, uint32_t *d_isCommon, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, uint32_t *d_segHead, uint32_t *d_segMedian)
{
	...
	uint32_t descriptor = d_lists[shortest_segstart];	//the first 32 bits of current segment
	++shortest_segstart;				//start of lower lb bits
	...
	if (tid < activeThreadNum) {
		...
		p = shortest_segment[pout * THREAD_NUM + tid];//DocID of shortest list to be searched
		...
		for (listIdx = 0; listIdx < tcount - 1; listIdx++) {
			...
			lb_start = segment_start + 1;
			eb_start = lb_start + ((lb * segment_size) >> 5) + (((lb * segment_size) & 31) > 0 ? 1 : 0);
			hb_start = eb_start + ((eb * en) >> 5) + (((eb * en) & 31) > 0 ? 1 : 0);
			lb_mask = (1U << lb) - 1;
			eb_mask = (1U << eb) - 1;
			hb_mask = (1U << hb) - 1;
			...
			if (p < cur_DocID)		//search in the first half
			{
			...
			else					//search in the second half
			{
				...
				middle = (segment_size >> 1);
			}


			ep = 0; 
			cur_en = 0;
			while (ep <= middle && cur_en < en) {
				idx = eb_start + ((cur_en * eb) >> 5);
				shift = ((cur_en * eb) & 0x1f);

				codeword = d_lists[idx] | (static_cast<uint64_t>(d_lists[idx + 1]) << 32);
				ep = (codeword >> shift) & eb_mask;

				++cur_en;
			}


			for ( ; middle < segment_size; ++middle, left += lb) {
				if (cur_DocID < p) {
					...
					codeword = d_lists[idx] | (static_cast<uint64_t>(d_lists[idx + 1]) << 32);
					cur_DocID = (codeword >> shift) & lb_mask;


					if (middle + 1 == ep && cur_en <= en) {
						idx = hb_start + (((cur_en - 1) * hb) >> 5);
						shift = ((cur_en - 1) * hb) & 0x1f;

						codeword = d_lists[idx] | (static_cast<uint64_t>(d_lists[idx + 1]) << 32);
						cur_DocID |= ((codeword >> shift) & hb_mask) << lb;
