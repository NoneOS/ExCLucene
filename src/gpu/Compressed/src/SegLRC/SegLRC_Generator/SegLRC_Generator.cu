...
#include "SegLRC_Generator_kernel.cu"
...
string result_dir = "/media/indexDisk/naiyong/result/SegLRC/Generator/";
...
void generateInd2Distance(const string &dataset) {
	...
	for (uint32_t i = 0; i < nListNum; ++i) {
		...
		SegLRC_Generator<<<nListLen / THREAD_NUM + 1, THREAD_NUM>>>(d_list, d_regressioninfo, d_distance, totalBlockNum, nListLen);
...
void RunGetRegressionInfo(const string &dataset) {
	...
	string strRegressionInfoList = regressioninfo_dir + dataset + ".regression_info_list";
	FILE *fRegressionInfoList = fopen(strRegressionInfoList.c_str(), "wb+");
	if (!fRegressionInfoList) {
		cout << "open binary file " << strRegressionInfoList << " failed;error:" << endl;
		exit(1);
	}
	...
	uint32_t nListNum = patind->sz_ind1 / sizeof(at_term_ind1_t);
	vector<regression_info_t> regressionInfoList(nListNum);
	...
	for (uint32_t i = 0; i < nListNum; ++i) {
		...
		// global regression info
		stat_info_t stinfo;
		memset(&stinfo, 0, sizeof(stat_info_t));
		getRegressionInfo(pList, nListLen, regressionInfoList[i], stinfo);


		// local regression info
		// all blocks except the last one
		uint32_t kBlockSize = THREAD_NUM;
		uint32_t kBlockNum = div_roundup(nListLen, kBlockSize);
		vector<stat_info_t> thisStatInfo(kBlockNum);
		for (uint32_t j = 0; j < kBlockNum - 1; ++j) {
			uint32_t offset = j * kBlockSize;
			getRegressionInfo(pList + offset, kBlockSize, regressionInfo[totalBlockNum + j], thisStatInfo[j]);
		}

		// tail blockSize
		uint32_t offset = (kBlockNum - 1) * kBlockSize;
		getRegressionInfo(pList + offset, nListLen - offset, regressionInfo[totalBlockNum + kBlockNum - 1], thisStatInfo[kBlockNum - 1]);

		// accumulate stat info
		if (nListLen > 1) {
			uint32_t idx = nListLen / interval;
			++count[idx];

			double dSumRSquare = 0, dSumContractionRatio = 0;
			for (uint32_t j = 0; j < kBlockNum; ++j) {
				dSumRSquare += thisStatInfo[j].dRSquare;
				dSumContractionRatio += thisStatInfo[j].dContractionRatio;
			}

			statInfo[idx].dRSquare += dSumRSquare / kBlockNum;
			statInfo[idx].dContractionRatio += dSumContractionRatio / kBlockNum;
		}


		totalBlockNum += kBlockNum;
	}
	// write result to binary file
	fwrite(&regressionInfoList[0], sizeof(regression_info_t), nListNum, fRegressionInfoList);
	fflush(fRegressionInfoList);

	fwrite(regressionInfo,  sizeof(regression_info_t), totalBlockNum, fRegressionInfo);
...
void runTest(int argc, char** argv) {
	...
	string result = result_dir + dataset + "_SegLRC_Generator.txt";
