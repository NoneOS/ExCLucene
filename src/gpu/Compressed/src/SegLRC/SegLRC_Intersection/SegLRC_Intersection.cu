...
#include "SegLRC_Intersection_kernel.cu"

using namespace std;


string dataset_dir = "/media/indexDisk/naiyong/dataset/";
string data_dir = "/media/indexDisk/naiyong/data/SegLRC/Compression/0/";
string regressioninfo_dir = "/media/indexDisk/naiyong/data/SegLRC/Generator/";
string result_dir = "/media/indexDisk/naiyong/result/SegLRC/Intersection/";
...
void allocateResource() {
	...
	CUDA_SAFE_CALL(cudaMalloc((void**)&d_fLinearInfo, 16380 * sizeof(float)));
...
batchInfo CPUPreprocess(unsigned char *&ptr_in, const unsigned char *ptr_end) {
	...
	while (ucntShortest_sum <= Threshold && ptr_in < ptr_end) {
		...
		for (uint32_t i = 0; i < tcount; ++i) {
			h_fLinearInfo[i + LinearInfo_offset] = (h_regression_info_list + tc.tno[i])->fSlope;
			h_fLinearInfo[tcount + i + LinearInfo_offset] = (h_regression_info_list + tc.tno[i])->fIntercept;
			h_fLinearInfo[tcount * 2 + i + LinearInfo_offset] = (h_regression_info_list + tc.tno[i])->nRangeLeft;
			h_fLinearInfo[tcount * 3 + i + LinearInfo_offset] = (h_regression_info_list + tc.tno[i])->nRangeRight;
		}
		...
		*(h_queries + queries_offset + 2) = LinearInfo_offset;
		memcpy(h_queries + queries_offset + 3, tc.ucnt, sizeof(uint32_t) * tc.tcount);
		memcpy(h_queries + queries_offset + 3 + tc.tcount, tc.uwlist, sizeof(uint32_t) * tc.tcount);
		memcpy(h_queries + queries_offset + 3 + 2 * tc.tcount, tc.segNum, sizeof(uint32_t) * tc.tcount);
		// copy ends
		...
		queries_offset += tc.tcount * 3 + 3;
		LinearInfo_offset += tc.tcount * 4;
		++nTotalQueryNum;
	}
...
void kernelInvoke(const batchInfo &bi) {
	...
	SegLRC_Intersection<<<bi.blockNum, THREAD_NUM>>>(d_lists, d_isCommon, d_queryID_perBlock, d_shortest_lists, d_segOffset, d_segHead, d_regression_info, d_base, d_fLinearInfo);
...
void releaseResource() {
	...
	free(h_regression_info_list);
	h_regression_info_list = NULL;
...
void Run(const string &dataset, unsigned char *ptr_in, unsigned char *ptr_end) {
	string result = result_dir + dataset + "_SegLRC_Intersection.txt";
...
void runTest(int argc, char **argv) {
	...
	string regression_info_list_file = regressioninfo_dir + dataset + ".regression_info_list";
	...
	FILE *fregression_info_list = fopen(regression_info_list_file.c_str(), "rb");
	if (!fregression_info_list) {
		cout << regression_info_list_file << " open failed\terr code: " << errno << endl;
		exit(1);
	}
	stat(regression_info_list_file.c_str(), &buf);
	regression_info_list_size = buf.st_size;
	h_regression_info_list = (regression_info_t*)malloc(regression_info_list_size);
	readFile((unsigned char*)h_regression_info_list, fregression_info_list);
	fclose(fregression_info_list);
