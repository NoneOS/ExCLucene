...
regression_info_t *h_regression_info_list;
uint32_t regression_info_list_size;
...
__global__ void SegLRC_Intersection(uint32_t *d_lists, uint32_t *d_isCommon, uint16_t *d_queryID_perBlock, uint32_t *d_shortest_lists, uint32_t *d_segOffset, uint32_t *d_segHead, regression_info_t *d_regression_info, int *d_base, float *d_fLinearInfo) {
	// shared
	...
	regression_info_t *regression_info = d_regression_info + segNum[0] + bid - startBlockId;
	float fSlope = regression_info->fSlope;
	float fIntercept = regression_info->fIntercept;	
	uint32_t nLeftRange = 0;
	uint32_t nRightRange = 0;
	...
	if (tid < activeThreadNum) {
		...
		DocID += (int)(tid * fSlope + fIntercept) + nBase;
...
//			uint32_t pivot = (s_tail - fIntercept) / fSlope; // slow
			...
			s_Border[tid] = rightBorder < ucnts[i] ? rightBorder : (ucnts[i] - 1);
		}
		else {  // left border
//			uint32_t pivot = (s_head - fIntercept) / fSlope;   // slow
			...
			s_Border[tid] = leftBorder > 0 ? leftBorder : 0;
		}
	...
	if (tid < activeThreadNum) {
		...
		for (uint32_t listIdx = 1; listIdx < tcount; ++listIdx) {
			...
			// slope, intercept and base of current longer list
			regression_info = (d_regression_info + segNum[listIdx] + left);
			...
			nBase = (d_base + segNum[listIdx])[left];

			int Pivot = ((float)DocID - fIntercept) / fSlope;
//			int Pivot = __fdividef(DocID - fIntercept, fSlope); // slow
			int leftNew = Pivot - nLeftRange;
			int rightNew = Pivot + nRightRange;
			left = leftNew > 0 ? leftNew : 0;
			right = rightNew < segment_size ? rightNew : segment_size - 1;
