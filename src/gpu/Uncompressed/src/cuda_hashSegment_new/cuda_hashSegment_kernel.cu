...
//global constants
#define GOV_SPEEDUP 3923
#define BD_SPEEDUP 22222
#define SPEEDUP GOV_SPEEDUP
...
const unsigned int at_trunc_count = 300000;  //约束的结果上限个数
const unsigned int baseSize = sizeof(unsigned int) * at_trunc_count * 128;  //最短桶长和的上限；因为现在是静态分配资源
...
unsigned int *d_hashSegInfo;  //segNum, segStart..., listLen
...
unsigned int *h_hashSegInfo;
unsigned int *h_hashSegOffset;
...
__global__ void mqSearch(unsigned int *d_constant, unsigned int *d_lists, unsigned int *d_isCommon, uint16_t *d_queryID_perBlock, unsigned int *d_hashSegInfo)
{
	//shared
	__shared__ unsigned int s_array[256];
	...
	unsigned int *ucnts, *uwlists, *hashOffset;
	//registers end
	...
	int listIdx = 0;
	unsigned char *pBloom;
	unsigned int nLenBloom;
	...
	if (threadIdx.x < activeThreadNum)
	{
		...
		for (listIdx = 1; listIdx < tcount; ++listIdx)
		{
			...
			nPivot = (p >> (d_hashSegInfo[hashOffset[listIdx]]));
			left = d_hashSegInfo[hashOffset[listIdx] + 1 + nPivot];
			right = d_hashSegInfo[hashOffset[listIdx] + 2 + nPivot] - 1;

//			isCommon[eleIndex] = (d_lists[uwlists[listIdx] + right]);
