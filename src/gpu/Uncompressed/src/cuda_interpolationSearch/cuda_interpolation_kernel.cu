...
__global__ void mqSearch(unsigned int *d_constant, unsigned int *d_lists, unsigned int *d_isCommon, uint16_t *d_queryID_perBlock)
{
	...
	*/
	...
	if (threadIdx.x < activeThreadNum)
	{
		...
		{
			unsigned int listHead = uwlists[listIdx];
			listLen = ucnts[listIdx];
			left = 0; right = listLen - 1;
			found = 0;
			int r = (d_lists[listHead + right]);
			int t = (d_lists[listHead + left]);

			while (t <= p && r >= p)
			{
				/*middle=left + int((float)(__umul24((right - left) ,(p - t))) / (float)(r - t));*/
				middle=left + int(((right - left)* (float)(p - t)) / (float)(r - t));
				/*middle = left + (int)((right - left) * (float)(p - t)) / (float)(r - t);*/
				q = (d_lists[listHead + middle]);
				
				if (p > q)
				{
					left = middle + 1;
					t = (d_lists[listHead + left]);
				}
				else if (p < q)
				{
					right = middle - 1;
					r = (d_lists[listHead + right]);
				}
				else
				{
					found = 1;
